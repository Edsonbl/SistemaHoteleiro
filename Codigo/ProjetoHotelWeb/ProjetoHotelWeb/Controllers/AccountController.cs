...
namespace IdentitySample.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController()
        {
            ...
            roleManager = new RoleManager<UserRole>(roleStore);
        }
        ...
        public ActionResult EditarUsuario(string userId)
        {
            ...
            bool userAdmin = User.IsInRole("Administrador");
            ...
            {
                userManager.RemoveFromRole(usuario.Id, role);
            }

            Char nivelDeAcesso = Convert.ToChar(Request.Params.Get("nivel"));
            if (nivelDeAcesso.Equals('A') && userAdmin)
            {
                userManager.AddToRole(usuario.Id, "Administrador");
            }
            else if (nivelDeAcesso.Equals('G') && userAdmin)
            {
                userManager.AddToRole(usuario.Id, "Gerente");
            }
            else if (nivelDeAcesso.Equals('R'))
            {
                userManager.AddToRole(usuario.Id, "Recepcionista");
            }
            else if (nivelDeAcesso.Equals('C'))
            {
        ...
        {
            User usuario = userManager.FindById(id);
            ViewBag.Funcionario = IPessoas.ResultadoUnicoFuncionario(usuario.idFuncionario);
            ViewBag.Role = userManager.GetRoles(id).First();
            return View(usuario);
        }
        ...
        public ActionResult Register(Register model)
        {
            ...
            {
                if (model.NivelDeAcesso.Equals('A') && userAdmin)
                {
                    userManager.AddToRole(user.Id, "Administrador");
                }
                else if (model.NivelDeAcesso.Equals('G') && userAdmin)
                {
        ...
        //================================= REGISTRO =====================================
        [ActionName("BuscaFuncionario")]
        public ActionResult BuscaFuncionario()
        {
            string filtro = Request.Params.Get("Filtro");
            string valor = Request.Params.Get("Consulta");
            List<PessoaModel> listaDeFuncinarios = FiltroConsulta(valor, filtro);
            if (listaDeFuncinarios.Count > 0)
            {
                ViewBag.Funcionarios = listaDeFuncinarios;
            }
            else
            {
                Console.WriteLine("Não veio nenhuma pessoa :(");
            }
        ...
        private List<PessoaModel> FiltroConsulta(string consulta, string filtro)
        {
            ...
            switch (Convert.ToInt32(filtro))
            {
                ...
                case 1:
                    ...
                    pessoas = IPessoas.ListarPorCpfCnpj(cpf).Where(Pessoa => Pessoa.isFuncionario = true).ToList<PessoaModel>();
                    break;

                case 2:
                    string nome = consulta;
                    pessoas = IPessoas.ListarPorNome(nome).Where(Pessoa => Pessoa.isFuncionario = true).ToList<PessoaModel>();
                    break;
